...
function graphCreator(data, size) {
    ...
    function all_nodes_mouseout() {
        if (_.UseNodeMouseOverContrast) {
            ...
            /*_.d3Links.attr("class", function (d) { return (d3.select(this).attr("class").replaceAll(" all-link-mouseover", "")); });*/
        }
    };
    ...
    function link_mouseover($links) { $links.attr("marker-end", getArrowHover).transition().attr("class", "link-hover"); };
    function link_mouseout($links) { $links.attr("marker-end", getArrow).transition().attr("class", "link"); };
